...
class FixedCloseIntervalTests(TestCase):
    def test_fixed_preclose_and_closing_auction_returns_use_stock_prices(self):
        ...
        self.assertEqual(round(result["pre_close_30m_return"], 6), 3.0)
        self.assertEqual(
            round(result["closing_auction_return"], 6),
            round((104 / 103 - 1) * 100, 6),
        )

    def test_fixed_close_intervals_are_missing_before_their_bars_exist(self):
        stock = pd.DataFrame(
        ...
        self.assertIsNone(result["pre_close_30m_return"])
        self.assertIsNone(result["closing_auction_return"])
