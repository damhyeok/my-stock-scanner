...
def render_rule_model_section(data: pd.DataFrame, selected_date: str) -> None:
    ...
    with st.expander('모델 조건 간단 설명'):
        st.markdown(
            ...
            '**표의 열 설명**\n\n'
            '- **종목명·종목코드**: 해당 종목의 이름과 6자리 코드\n'
            '- **Price Action**: 통과 유형. A=눌림 반등, B=고점 돌파, C=지지 회복\n'
            '- **일봉 CLV**: 당일 저가~고가 중 종가 위치. 100%에 가까울수록 고가 부근 마감\n'
            '- **14:30 이후 수익률**: 14:30 가격 대비 이후 종가 변화율\n'
            '- **오후 CLV**: 오후 구간의 저가~고가 중 마지막 가격 위치\n'
            '- **업종 대비 5일 RS**: 종목 5일 수익률에서 업종 대표 수익률을 뺀 값\n'
            '- **시장 대비 20일 RS**: 종목 20일 수익률에서 전체 시장 대표 수익률을 뺀 값\n'
            '- **ATR14**: 최근 14일 평균 변동폭. 높을수록 가격 움직임이 큼\n'
            '- **RS 품질**: 업종 비교가 가능했는지, 전체 종목으로 대신 비교했는지 표시\n'
            '- **전체 기술단계 상태**: 아직 통과하지 못한 단계 표시. `탈락: 오후 흐름`이면 오후 흐름 때문에 최종 탈락\n'
            '- **판정**: 모든 기술조건 통과 여부와 시장강도 수동판단 대기 상태\n\n'
